...
from ANN_UTADIS.threshold_layer import ThresholdLayer
...
class NormLayer(nn.Module):
    ...
    def predict(self, input: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of the module and binarization of the prediction.

        Args:
            x (torch.Tensor): Input tensor.

        Returns:
            torch.Tensor: Binarized (signum function) output tensor.
        """
        self.out = self.method_instance(input)
        ...
        self.out = (self.out - zero) / (one - zero)
        return (self.thresholdLayer(self.out)>0).int()